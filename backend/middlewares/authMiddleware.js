...
console.log('DEBUG AUTH MIDDLEWARE: JWT_SECRET en middleware (longitud):', JWT_SECRET?.length); // <-- DEBUG
...
const authenticateToken = (req, res, next) => {
    ...
    if (!token) {
        console.log('DEBUG AUTH: No se encontró token en el encabezado Authorization.');
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        ...
        console.log('DEBUG AUTH: ID de usuario desde token (req.user.id):', req.user.id); // <-- DEBUG
